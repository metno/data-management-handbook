...
Clone the link:https://github.com/metno/py-mmd-tools.git[py-mmd-tools] repo and
make a local installation with, e.g., `pip install .`. This should bring in all
needed dependencies (we recommend using a virtual environment).
...
Please coordinate with the Data Manager or the DMG if you intend to push MMD
files thorugh the queue. If you post an MMD file, make sure to check that it
has been added to the metadata catalogue (see <<test-mmd-ingested>>).

[[push-mmd-file]]
===== Push the MMD xml file to the discovery metadata catalogue

If you produce data that does not need to go through the event queue, the Data Manager or someone in the DMG can help you with the following:
