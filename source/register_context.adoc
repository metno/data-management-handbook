...
=== Metadata Controlled Production

In order to make a dataset findable, a dataset must be registered in a
searchable catalog with appropriate metadata.
...
(1) the event queue or (2) the Discovery Metadata Catalog Ingestor (DMCI)
service. Typically, metadata about operational data (i.e., data that are
...
. Push some test metadata to the event queue in dev (TODO: add section about how to publish to the event queue, and link it from here)
. Check that messages are coming through the event queue (TODO: add section about how to use the event queue, and link it from here)
. Check that the MMD file has been added to csw (see <<test-mmd-ingested>>)
