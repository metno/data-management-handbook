...
Make sure you have one valid api-key to publish an event to MMS. 

Push the Product Event to MMS:
  Include the arguments required to make the link:https://github.com/metno/MMS/blob/main/messages.md[Product Event]
  in the post command.

[source]
----
  --jobname=test --product=test --product-location=test-location --production-hub=https://mms-api.s-enda-*.k8s.met.no --MMD="$(cat /path/to/MMdfile.xml)" --counter=1 --ntotal=1 --reftime=2023-11-02T12:02:46Z --event-interval=3
----   

`ntotal` is the `TotalCount` and `event-interval` is used to make 
`NextEventAt` in Product Event. 
...
Make sure you have one valid cred file to subscribe to a queue .
...
The `MMD-Agent` is subscribed to all the messages coming through the event queue. If the messsage contains a MMD payload, it will post the MMD file to the DMCI API, which stores the metadata in the catalog.
...
* Find the datasets having a given `metadata_identifier = no.met:4a34334b-b384-48b3-b846-76807bc006`: `https://csw.s-enda*.k8s.met.no/csw?service=CSW&version=2.0.2&request=GetRecordById&oxml&outputSchema=http://www.isotc211.org/2005/gmd&elementsetname=full&id=no.met:4a34334b-b384-48b3-b846-76807bc006`
