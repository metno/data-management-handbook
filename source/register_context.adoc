...
. Create NATS JetStream NSE user for publishing events: SHAMLY
. Create NATS JetStream NSE user for subscribing to events: SHAMLY
. Create a link:https://github.com/metno/MMS/blob/main/messages.md[Product Event]: SHAMLY
. Push the Product Event to MMS in staging: SHAMLY 
. Check that messages are coming through the event queue: SHAMLY
...
SHAMLY: write what happens when events contain MMD strings
...
. Check that the MMD file has been added to the catalog services in staging (see <<test-mmd-ingested>>).
...
. Check that the MMD file has been added to the catalog services in production (see <<test-mmd-ingested>>).
