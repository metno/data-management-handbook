...
If you also posted an MMD file, check that this has been added to the metadata
...
. Push to production (the official catalogue) for final publication: `curl --data-binary "@<PATH_TO_MMD_FILE>" https://dmci.s-enda.k8s.met.no/v1/insert`
