...
with nc.Dataset(sys.argv[1], "a") as f:
    ...
    try:
        ...
        warnings.warn("Warning: conversion of first timestep to datetime in ISO format failed, setting time_coverage_start as 1900-01-01 00:00:00")
        f.time_coverage_start = datetime.strptime('1900-01-01', '%Y-%m-%d').replace(tzinfo=timezone.utc).isoformat()
