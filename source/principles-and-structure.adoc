...
Granularity is a constant challenge in distributed data management. Quite often
the discovery level information provided is only useful for human
...
than a potential data consumer. An example on this is, e.g., profiles of
temperature and salinity data from research cruises. These data are often
...
cruises, the work load on the data consumer is increased, without any benefit.
...
in, e.g., numerical models, satellite calibration, and validation activities.
This means that data from different stations and instruments are kept in
separate datasets/files, and not in the same. These datasets can then be
combined into networks, field activities, research cruises, etc., using
parent/child relations or just by tagging them with specific keywords.
...
The same is also valid for feature types (e.g., time series of point
observations or profiles), which should not be mixed. For example, weather
...
time. This also helps to simplify the process of establishing automated work
flows, e.g., in support of the emerging European data spaces.
====

Recommendations:

* Always publish data at the highest possible functional granularity (i.e., not individual
measurements, but neither several stations combined in one dataset)
...
* If there is a need to reference datasets to collection work (e.g., research cruises or field work), establish this reference through metadata keywords or trough parent-child relations, allowing data consumers to collect data based on content and spatio-temporal traits.
...
controlled vocabularies in, e.g., the MMD standard <<RD7>>.
